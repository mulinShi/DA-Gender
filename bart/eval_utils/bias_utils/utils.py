...
def save_back_json(file, filename):
...
def attention_mask_creator(input_ids):
    ...
    for sent in input_ids:
        segments_ids = [int(t > 1) for t in sent]
...
def new_input_pipeline(sequence, tokenizer, MAX_LEN):
    
    dic = tokenizer(sequence,
                 add_special_tokens=True,
                 truncation=True, 
                 max_length=MAX_LEN, 
                 padding="max_length")

    return torch.tensor(dic['input_ids']), torch.tensor(dic['attention_mask'])
...
def prob_with_prior_for_attr(pred_AM, pred_TAM, input_ids_AM, original_ids, tokenizer):
    pred_AM = pred_AM.cpu()
    pred_TAM = pred_TAM.cpu()
    input_ids_AM = input_ids_AM.cpu()

    probs = []
    for doc_idx, id_list in enumerate(input_ids_AM):
        # see where the masks were placed in this sentence
        mask_indices = np.where(id_list == tokenizer.mask_token_id)[0]

        res = 1
        # print(original_ids[doc_idx])
        # print(len(mask_indices))
        assert len(mask_indices) > 0 and len(mask_indices) < 6
        for indic in mask_indices:
            target_id = original_ids[doc_idx][indic]
            target_prob = pred_AM[doc_idx][indic][target_id].item()
            prior = pred_TAM[doc_idx][indic][target_id].item()
            res *= (target_prob / prior)
        probs.append(np.log(res))

    return probs

def model_evaluation_by_attr(eval_df, tokenizer, model, device):
    ...
    assert eval_tokens_AM.shape == eval_attentions_AM.shape
    assert eval_tokens_TAM.shape == eval_attentions_TAM.shape

    eval_batch = 20
    eval_data = TensorDataset(eval_tokens_AM, eval_attentions_AM,
    ...
    for step, batch in enumerate(eval_dataloader):
        b_input_AM = batch[0].to(device)
        b_att_AM = batch[1].to(device)
        b_input_TAM = batch[2].to(device)
        b_att_TAM = batch[3].to(device)

        with torch.no_grad():
            outputs_AM = model(b_input_AM,
                               attention_mask=b_att_AM)
            outputs_TAM = model(b_input_TAM,
                                attention_mask=b_att_TAM)
            predictions_AM = softmax(outputs_AM[0], dim=2)
            predictions_TAM = softmax(outputs_TAM[0], dim=2)

        assert predictions_AM.shape == predictions_TAM.shape

        # calculate associations
        associations = prob_with_prior_for_attr(predictions_AM,
                                       predictions_TAM,
                                       b_input_AM,
