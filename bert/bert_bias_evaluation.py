...
def parse_arguments():
    ...
    parser.add_argument('--model_path', help='which model to use', required=False, default = 'bert-base-uncased')
...
if __name__ == '__main__':
    ...
    corpus, tmps_type, temp_size = convert_tmps_to_sents(tmp_path, eval_type,
                                                     simple = True)
    sent2emb, model_type = get_embeddings_and_probs(corpus, tmps_type, temp_size,
                           model_type = model_path, subspace_path = subspace_path)
    corpus = add_assos(corpus, tmps_type, temp_size, model_type = model_path, sent2emb = sent2emb)
    ...
    with open("bert_bias_eval_res.json", 'a') as f:
