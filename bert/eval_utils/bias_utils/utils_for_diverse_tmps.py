...
def get_only_embeddings(sents, tokenizer, model, device, max_len):
    # logging.info("******2 The type of used model is {}".format(type(model)))
    ...
    for step, batch in enumerate(tqdm(eval_dataloader)):
        ...
        with torch.no_grad():
            embs = model(input_ids = b_input, attention_mask=b_att,
                output_hidden_states=True).hidden_states[-1]
...
def get_embedding_with_probs(sents, indexs, genders_list, tokenizer, model, device, max_len_eval, model_type, gender_dir):
    ...
    for step, batch in enumerate(tqdm(eval_dataloader)):
        ...
        with torch.no_grad():
            outputs = model(input_ids = b_input, attention_mask=b_att,
            ...
            if gender_dir is not None:
                ...
                outputs_before_cls = model.bert(input_ids = b_input, attention_mask=b_att).last_hidden_state
