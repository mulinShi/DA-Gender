...
def get_embs_for_autoregressive_models(sents, tokenizer, model, device, need_reload = False):

    print("****** need_reload : ", need_reload)
    ...
    for i, sent in enumerate(tqdm(sents)):
        ...
        att_ids = inputs['attention_mask'].to(device)
        # tokens_tensor = torch.tensor(
        #             tokens).unsqueeze(0).cuda()
        # print("* The device is ", device)
        # print("* The input_ids.device is ", input_ids.device)
        # print("* The att_ids.device is ", att_ids.device)
        with torch.no_grad():
            # print("* The model.module.device is ", model.module.device)
            embs = model(input_ids = input_ids, attention_mask = att_ids, output_hidden_states=True).hidden_states[-1][:, -1, :].cpu().detach().numpy()
        # assert list(embs.shape) == [1, 1024]
