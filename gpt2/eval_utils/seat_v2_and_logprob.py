...
from bias_utils.utils_for_diverse_tmps import model_evaluation_by_seat, model_evaluation_by_log_prob_score\
                                                , get_only_embeddings, get_embedding_with_probs, get_embs_for_autoregressive_models
import pickle
import math
import argparse
...
def comput_asso_for_autoregressive(corpus, tmps_type, temp_size, device = None, model_type = None, subspace_path = None, tokenizer = None):
    gender_dir = None
    ...
    TM_list = list(set([i for i in corpus.Sent_TM]))

    all_sents_list = AM_list + TM_list
    ...
    corpus = corpus.assign(seat = seats)
    # corpus.to_csv('res/{}_{}_{}_res.tsv'.format(model_path, tmps_type, temp_size), sep='\t')
    return corpus, sent2emb
...
def parse_arguments():
    ...
    parser.add_argument('--model_type', help='which model to use', required=False, default = 'gpt2')
...
if __name__ == '__main__':
    ...
    corpus, tmps_type, temp_size = convert_tmps_to_sents('data/simple_patterns',\
                                                     simple = True)
    corpus, sent2emb = comput_asso_for_autoregressive(corpus, tmps_type, temp_size, model_type = model_type\
        , model_path = model_path, subspace_path = subspace_path)

    debiasing_effetcs = {}

    seat_effect_size = get_effect_size(corpus)
    debiasing_effetcs['seat_effect_size'] = seat_effect_size
    ...
    debiasing_effetcs['seat_corr'] = stats.spearmanr(real_probs, computed_assos)
    
    print("* res: ", debiasing_effetcs)
    with open('res/{}_{}_debiasing_effetcs.json'.format(debiasing_type, model_type), 'w') as f:
        json.dump(debiasing_effetcs, f)

    # corpus, tmps_type, temp_size = convert_tmps_to_sents()
    # sent2emb = get_embeddings_and_probs(corpus, tmps_type, temp_size)
    # corpus = add_assos(sent2emb, corpus, tmps_type, temp_size)